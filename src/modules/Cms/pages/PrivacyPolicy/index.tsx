...
const PrivacyPolicy = () => {
  ...
  const fetchData = async () => {
    const data = await getApi(`${REACT_APP_API_URL}/cms-page-section`, {
      params: {
        sectionName: CMSEnum.PrivacyPolicy,
      },
    });
  ...
  const privacyData = !_.isUndefined(responseData?.privacyPolicy)
    ? formatCMSObjectData({ data: responseData?.privacyPolicy ?? [] })
    : {};

  const { description, point_data_array, summary_keyPoints, title } =
    privacyData as unknown as PrivacyPolicyProps;
                      ...
                      <div className="inner">
                        ...
                        <ul>
                          {point_data_array?.map((item, index) => (
                            <li key={`privacy_policy_q_no${index + 1}`}>
                              <Link
                                to="#QNASection"
                                onClick={() => {
                                  setQnaData(item);
