...
const CMSCTA = ({
  ...
  return (
    <section
      className={`home-cta fill ${className || ''} ${variant === '1' ? 'v1' : variant === '2' ? 'v2' : variant === '3' ? 'v3' : ''}`}
    >
            ...
            <div className="inner">
              ...
              <div className="btn btn-black-border">
                <Link to={linkURL ?? (data?.cta_button_url as string)}>
                  {linkText ?? String(data?.cta_button_title ?? data?.button_text)}
