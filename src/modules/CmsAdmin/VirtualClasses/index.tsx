...
const CMSVirtualClassesAdmin = () => {
  ...
  const dataFEtch = async () => {
    const response = await getApi(`${REACT_APP_API_URL}/cms-page-section/${id}`, {
      params: { sectionName: KeysEnum.VirtualClass, view: true },
    });
    const initialValues = {} as LangueKeyValueProps;
    setResponseData(response.data[KeysEnum.VirtualClass]);

    response.data[KeysEnum.VirtualClass].forEach(
      ({
        language,
        field_name,
        field_value,
  ...
  return (
    <>
      <PageHeader
        title={t('Cms.virtualClass.title')}
      ...
      >
        <Breadcrumbs
          items={[
            ...
            {
              label: t('Cms.virtualClass.title'),
              url: '/',
            },
          ]}
          variant="arrow"
      ...
      <div className="content-base">
        ...
        <Card title={t('Cms.virtualClass.title')} isGray>
          <CommonSection
...
export default CMSVirtualClassesAdmin;
