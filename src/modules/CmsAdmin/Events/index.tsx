...
// ** Validation **
import { CMSEventValidationSchema } from './validation';
...
const CMSEvents = () => {
  ...
  const fetchEventData = async () => {
    const { data } = await getApi(`/cms-page-section/${id}`, {
      params: {
        sectionName: KeysEnum.Event,
        view: true,
      },
    });
    setResponseData(data?.cmsEvent);
    const initialValues = {} as LangueKeyValueProps;
    data.cmsEvent?.forEach(
      ({
        language,
        field_name,
        field_value,
  ...
  return (
    <div className="relative">
      <PageHeader
        title={t('Events.PageHeader.Title')}
      ...
      >
        <Breadcrumbs
          items={[
            ...
            {
              label: t('Events.PageHeader.Title'),
              url: '/',
            },
          ]}
          variant="arrow"
          ...
          <div className="">
            <div className="">
              <Formik
                ...
                validationSchema={CMSEventValidationSchema()}
                onSubmit={(values) => {
                  onSubmit(values);
                }}
                enableReinitialize
                  ...
                  return (
                    <Form id="scrollable-form">
                      <div className="bg-white m-10 p-10 rounded-xl">
                        <InputField
                          name="hashTag"
                          label={t('Events.TitleHashTag')}
                          placeholder={t('Events.TitleHashTag.PlaceHolder')}
                          type="text"
                          parentClass="mb-5"
                        />
                        <InputField
