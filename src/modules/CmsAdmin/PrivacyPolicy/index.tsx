...
const PrivacyPolicyAdmin = () => {
  ...
  const dataFEtch = async () => {
    const response = await getApi(`${REACT_APP_API_URL}/cms-page-section/${id}`, {
      params: { sectionName: KeysEnum.PrivacyPolicy, view: true },
    });
    const initialValues = {} as LangueKeyValueProps;
    setResponseData(response.data[KeysEnum.PrivacyPolicy]);

    response.data[KeysEnum.PrivacyPolicy].forEach(
      ({
        language,
        field_name,
        field_value,
  ...
  return (
    <>
      <PageHeader
        title={t('Cms.privacyPolicy.title')}
      ...
      >
        <Breadcrumbs
          items={[
            ...
            {
              label: t('Cms.privacyPolicy.title'),
              url: '/',
            },
          ]}
          variant="arrow"
      ...
      <div className="content-base">
        ...
        <Card title={t('Cms.privacyPolicy.title')} isGray>
          <CommonSection
